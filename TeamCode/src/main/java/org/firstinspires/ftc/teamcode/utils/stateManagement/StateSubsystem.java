...
public abstract class StateSubsystem<E extends Enum<E>> extends Subsystem {
    ...
    protected ArrayList<Transition<E>> transitions;
    private E state;
    ...
    protected void setTransitionFunction(E from, E to, Runnable function) {
        transitions.add(new Transition<>(from, to, function));
    }
    protected void setTransitionFunction(Transition.Type type, E transition, Runnable function) {
        transitions.add(new Transition<>(type, transition, function));
    }
    ...
    public void setState(E newState) {
        ...
        E oldState = state;
        state = newState;
        Transition<E> currentTransition = new Transition<>(oldState, newState, () -> {});

        for (Transition<E> transition : transitions) {
            if (transition.equals(currentTransition))
                transition.function.run();
        }
    }
}